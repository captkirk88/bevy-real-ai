use bevy::prelude::*;
use bevy_real_ai::context::{ContextGatherRequest, AiSystemContextStore};
...
fn gather_on_request_collects_nearby_entity_context() {
    ...
    store.add_system(
        |ai_entity: bevy_real_ai::context::AiEntity,
         characters: Query<(Entity, &Character, &Transform), With<bevy_real_ai::context::AIAware>>| 
    ...
    eprintln!("Request queue length before: {}", app.world().resource::<bevy_real_ai::dialogue::DialogueRequestQueue>().len());
    ...
    let resp = match bevy_real_ai::test_helpers::ask_ai_and_wait_result(&mut app, requester, "Where is the sword?", 10000) {
        Ok(r) => r,
        Err(e) => e,
    };
