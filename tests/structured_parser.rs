...
fn real_ai_parses_typed_spawn_action() {
    ...
    match result {
        Ok((parsed, response)) => {
            ...
            assert_eq!(pending.actions.len(), 1, "Should have queued exactly one action");
...
fn schema_description_is_generated_correctly() {
    ...
    assert!(schema.contains("name"), "Schema should contain 'name' field");
    assert!(schema.contains("x"), "Schema should contain 'x' field");
    assert!(schema.contains("y"), "Schema should contain 'y' field");
    assert!(schema.contains("string"), "Schema should contain 'string' type");
    assert!(schema.contains("integer"), "Schema should contain 'integer' type");
}
...
```
"#;
    let result: SpawnAction = extract_and_parse_json(markdown).expect("should parse from code block");
