...
fn preprogrammed_response_is_immediate() {
    ...
    let e = app
        ...
        .spawn((
            AI,
            DialogueReceiver::new_with_preprogrammed("Hello, traveler"),
        ))
        .id();

    // Ask AI and wait for response using test helper
    let resp = bevy_real_ai::test_helpers::ask_ai_and_wait(&mut app, e, "ignored", 10)
...
fn mock_ai_generates_response() {
    ...
    let resp = bevy_real_ai::test_helpers::ask_ai_and_wait(&mut app, e, "Say hi", 50)
        .expect("expected response");
...
fn ai_action_block_is_parsed_and_stored() {
    ...
    // Register a registry handler that spawns TestSpawned so we can assert it ran
    {
        ...
        registry.register(
            "spawn_entity",
            move |In(_event): In<bevy_real_ai::actions::AiActionEvent>, mut cmds: Commands| {
    ...
    let receiver = app
        .world()
        .get::<DialogueReceiver>(e)
        .expect("receiver exists");
    assert_eq!(
        receiver.last_response.as_deref(),
        Some(r#"{"name": "spawn_entity", "params": {"prefab": "goblin", "x": 2.0}}"#)
    );
    assert_eq!(receiver.actions.len(), 1);
    let action = &receiver.actions[0];
    assert_eq!(action.name, "spawn_entity");
    assert_eq!(
        action.params["prefab"],
        serde_json::Value::String("goblin".to_string())
    );
