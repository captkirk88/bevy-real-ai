...
pub fn assert_ai_response<F: Fn(&str) -> bool>(app: &mut App, entity: Entity, prompt: &str, max_updates: usize, predicate: F) {
    match ask_ai_and_wait(app, entity, prompt, max_updates) {
        Some(resp) => assert!(predicate(&resp), "AI response did not match predicate: {}", resp),
...
pub fn ask_ai_with_context(app: &mut App, entity: Entity, contexts: &[&str], prompt: &str, max_updates: usize) -> Option<String> {
